...
const FACTORY_NAME = 'Carrier';
const ROUTE = '/carriers';

describe('CarrierController.js', () => {
  beforeEach(async () => {
  ...
  it(`should return specific ${FACTORY_NAME} by name`, async () => {
    const registers = await factory.createMany(FACTORY_NAME, 10);

    const response = await request(app)
      .get(ROUTE)
      .query({
        q: registers[5].dataValues.name,
      })
      .set('Authorization', `Bearer ${token}`);
  ...
  it(`should not be able to update ${FACTORY_NAME} without name`, async () => {
    const { dataValues: register } = await factory.create(FACTORY_NAME);
